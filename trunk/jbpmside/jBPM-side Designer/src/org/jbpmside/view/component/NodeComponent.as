	...
	public class NodeComponent extends ShapeComponent
	{
		...
		public override function mouseClickHandler(event:MouseEvent):void{
			...
			if(isConnectSelect()){
				...
				model.dispatchEvent(connectEvent);
			}else{
				selected();
			}
		}
		...
		private function isConnectSelect():Boolean{
			if(model.selectedMode==TheModel.SELECTED_TRANSITION){
		...
		public function updateConnectionPositions():void{
			...
			for(var i:int=0;i<_leaveConnections.length;i++){
				var connection:ConnectionComponent=_leaveConnections[i] as ConnectionComponent;
				connection.refreshPosition();
			}
		}
