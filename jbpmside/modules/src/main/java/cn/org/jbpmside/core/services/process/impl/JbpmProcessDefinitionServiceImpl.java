package cn.org.jbpmside.core.services.process.impl;
...
import cn.org.jbpmside.core.util.db.JbpmBaseJdbcDAO;
 ...
 * Time: 22:09:59
 */
public class JbpmProcessDefinitionServiceImpl implements JbpmProcessDefinitionService {
    private static Log log = LogFactory.getLog(JbpmProcessDefinitionServiceImpl.class);
     ...
     * @return 流程定义的列表
     */
    public List<ProcessDefinition> getAllLatestProcessDefinitionLists(Page page) {
        return (List) jbpmBaseJdbcDAO.findByOutQuery(JBPMSIDE_QUERY_ALL_PROCESSDEFINITIONS, null, page);
    }
     ...
     * @return 流程定义列表
     */
    public List<ProcessDefinition> getLatestProcessDefinitionListsByPage(Page page) {
        ...
        try {
            ...
            Query query = hibernateSession.getNamedQuery(JBPMSIDE_QUERY_ALL_PROCESSDEFINITIONS);
            List list = query.list();
            for (Object obj : list) {
                if (obj instanceof ProcessDefinition) {
                    ProcessDefinition processDefinition = (ProcessDefinition) obj;
                    String processDefinitionName = processDefinition.getName();
                    ProcessDefinition previous = processDefinitionsByName.get(processDefinitionName);
                    if ((previous == null)
                ...
                if (page == null)
                    throw new NullPointerException("page is null in class=[JbpmProcessDefinitionServiceImpl]"
                            + "method=[getLatestProcessDefinitionListsByPage]");
                processDefinitions = new ArrayList<ProcessDefinition>(processDefinitionsByName.values());
                ...
                processDefinitions = processDefinitions.subList(fromIndex, toIndex); //分页以后的list
            }
        } catch (Exception e) {
            if (log.isDebugEnabled())
                e.printStackTrace();
            else
                log.error("couldn't find latest versions of process definitions", e);
