package cn.org.jbpmside.core.services.task.impl;
...
public class JbpmWorklistQueryServiceImpl implements JbpmWorklistQueryService {
     ...
     *
     * @param tokenid 根tokenid
     * @return TaskInstanceDTO的集合
     */
    public Collection<TaskInstanceDTO> getDetailTaskListByTokenid(long tokenid) {
        ProcessInstance processInstance = jbpmProcessInstanceService.getProcessInstance(tokenid);
        Collection list = processInstance.getTaskMgmtInstance().getTaskInstances();
     ...
     * @return TaskInstanceDTO的集合
     */
    private Collection<TaskInstanceDTO> queryListBySqlType(String sqlPath, String userId, Page page) {
     ...
     *
     * @param list TaskInstance对象的集合
     * @return TaskInstanceDTO对象的集合
     */
    private Collection<TaskInstanceDTO> converToDTO(Collection<TaskInstance> list) {
        ...
        try {
            for (Object taskInstance : list) {
                if (taskInstance instanceof TaskInstance) {
                    ...
                    taskInstanceDTO.setName(newtaskInstance.getTask().getName());
                    ...
                    taskInstanceDTO.setTokenId(newtaskInstance.getToken().getId());
                    taskInstanceDTO.setTaskId(newtaskInstance.getId());
                    taskInstanceDTO.setTaskMgmtInstanceId(newtaskInstance.getTaskMgmtInstance().getId());
                    ProcessInstance processIntance = jbpmProcessInstanceService.getProcessInstance(newtaskInstance.getToken().getId());
                    taskInstanceDTO.setProcessDefinitionName(processIntance.getProcessDefinition().getName());
                    taskInstanceDTO.setProcessDefinitionVersion(processIntance.getProcessDefinition().getVersion() + "");
