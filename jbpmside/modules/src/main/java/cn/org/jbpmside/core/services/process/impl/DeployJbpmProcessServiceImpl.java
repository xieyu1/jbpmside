 ...
 * Time: 16:53:16
 */
public class DeployJbpmProcessServiceImpl implements IDeployJbpmProcessService {
    private static Log log = LogFactory.getLog(DeployJbpmProcessServiceImpl.class);
    ...
    private void importJbpmTemplate(String jbpmsideProcessFileName) {
        ...
        try {
            ...
            if (file.exists()) {
                ...
                throw new RuntimeException("The definition xml file [" + jbpmsideProcessFileName + "] doesn't exist!");
            }
        } catch (Exception e) {
            if (log.isDebugEnabled())
                e.printStackTrace();
            else
                log.error("The definition xml file [" + jbpmsideProcessFileName + "] doesn't exist!", e);
    ...
    static {
        ...
        if (filePath == null) {
            log.info("没有指定环境变量JBPMSIDE_HOME。进入%tomcat%的目录下的bin中的startup.bat中设置set JBPMSIDE_HOME=../../../../conf/jbpmside-home（你的路径）");
        } else {
